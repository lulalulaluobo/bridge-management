...
import { foodCategories, storageLocations, type FoodBatchWithStatus, type ProposalAction } from "@/lib/inventory/types";
...
export function InventoryDashboard({ initialBatches, initialCredentials, initialPreferences, vapidPublicKey }: { initialBatches: FoodBatchWithStatus[]; initialCredentials: CredentialSummary[]; initialPreferences: FoodPreferences; vapidPublicKey: string }) {
  ...
  async function askAgent(message: string) {
    ...
    try {
      ...
      const data = await response.json() as { message?: string; speech?: string; committed?: unknown; error?: string };
  ...
  return <main className={tab === "today" ? "h-[100dvh] overflow-hidden bg-[#f3f4f0] text-[#17231f]" : "min-h-[100dvh] bg-[#f3f4f0] pb-36 text-[#17231f]"}>
    <div className={`mx-auto w-full max-w-xl px-5 pt-[max(1.2rem,env(safe-area-inset-top))] ${tab === "today" ? "h-full" : ""}`}>
      ...
      {tab === "inventory" && <InventoryView batches={batches} onAdd={() => setShowAdd(true)} onConsume={consume} onEditStart={setEditingBatch} onDelete={(batch) => void queueAction({ type: "soft_delete_batch", batchId: batch.id })} />}
...
function InventoryView({ batches, onAdd, onConsume, onEditStart, onDelete }: { batches: FoodBatchWithStatus[]; onAdd: () => void; onConsume: (batch: FoodBatchWithStatus) => void; onEditStart: (batch: FoodBatchWithStatus) => void; onDelete: (batch: FoodBatchWithStatus) => void }) {
...
function BatchRow({ batch, first, onConsume, onEdit, onDelete }: { batch: FoodBatchWithStatus; first: boolean; onConsume: () => void; onEdit: () => void; onDelete: () => void }) {
  ...
  return <article className={`${first ? "" : "border-t border-[#edf0eb]"} flex items-center gap-2 px-4 py-3`}><button type="button" onClick={onEdit} className="min-w-0 flex-1 text-left active:opacity-65" aria-label={`编辑 ${batch.name}`}><span className="flex items-center gap-1.5"><span className="truncate font-semibold tracking-[-.015em]">{batch.name}</span><span className={`shrink-0 rounded-full px-2 py-0.5 text-[11px] font-semibold ${tone}`}>{status}</span><span className={`shrink-0 rounded-full px-2 py-0.5 text-[11px] font-semibold ${batch.opened ? "bg-[#fce8cd] text-[#995f13]" : "bg-[#eef1ee] text-[#758278]"}`}>{batch.opened ? "已开封" : "未开封"}</span></span><span className="mt-1 block truncate text-sm text-[#66756d]">{batch.quantity}{batch.unit} · {batch.storageLocation} · {batch.expiresAt}到期</span></button><div className="flex shrink-0 items-center gap-1"><ActionIcon label={`编辑 ${batch.name}`} icon="pencil" onClick={onEdit} /><ActionIcon label={`消耗 ${batch.name}`} icon="consume" onClick={onConsume} /><ActionIcon label={`删除 ${batch.name}`} icon="trash" onClick={onDelete} danger /></div></article>;
}

function ActionIcon({ label, icon, onClick, danger = false }: { label: string; icon: IconName; onClick: () => void; danger?: boolean }) { return <button type="button" onClick={onClick} aria-label={label} className={`grid h-9 w-9 place-items-center rounded-full bg-[#eff2ee] active:scale-95 ${danger ? "text-rose-700" : "text-[#405148]"}`}><Icon name={icon} /></button>; }

function Sheet({ title, onClose, children, compact = false }: { title: string; onClose: () => void; children: React.ReactNode; compact?: boolean }) { return <div className="fixed inset-0 z-50 flex items-start justify-center bg-[#17231f]/20 px-4 pt-[max(1rem,env(safe-area-inset-top))] backdrop-blur-[2px]" role="dialog" aria-modal="true"><section className={`${compact ? "max-h-[56dvh]" : "max-h-[calc(100dvh-2rem)]"} w-full max-w-md overflow-y-auto rounded-[26px] bg-[#fbfcf9] px-5 py-4 shadow-2xl`}><div className="mb-4 flex items-center justify-between"><h2 className="text-lg font-bold tracking-[-.03em]">{title}</h2><button type="button" onClick={onClose} className="grid h-9 w-9 place-items-center rounded-full bg-[#eef0eb]" aria-label="关闭"><Icon name="close" /></button></div>{children}</section></div>; }
