...
const today = new Intl.DateTimeFormat("zh-CN", { timeZone: "Asia/Shanghai", month: "long", day: "numeric", weekday: "short" }).format(new Date());
...
export function InventoryDashboard({ initialBatches, initialCredentials, initialPreferences, vapidPublicKey }: { initialBatches: FoodBatchWithStatus[]; initialCredentials: CredentialSummary[]; initialPreferences: FoodPreferences; vapidPublicKey: string }) {
  ...
  return <main className="min-h-[100dvh] bg-[#f3f4f0] pb-36 text-[#17231f]">
    <div className="mx-auto w-full max-w-xl px-5 pt-[max(1.2rem,env(safe-area-inset-top))]">
      <header><p className="text-[13px] font-medium tracking-wide text-[#64736c]">{today}</p><h1 className="mt-1 text-[28px] font-bold leading-none tracking-[-0.045em]">冰箱</h1></header>
...
function TodayView({ messages, chatInput, busy, agentPhase, setAgentPhase, setChatInput, sendMessage, recognizeImage, transcribe, voiceError, voiceReplies, toggleVoiceReplies, openAdd }: { messages: Array<{ role: "assistant" | "user"; content: string }>; chatInput: string; busy: boolean; agentPhase: AgentPhase; setAgentPhase: (phase: AgentPhase) => void; setChatInput: (value: string) => void; sendMessage: (event: React.FormEvent<HTMLFormElement>) => void; recognizeImage: (file: File) => void; transcribe: (audio: Blob) => void; voiceError: (message: string) => void; voiceReplies: boolean; toggleVoiceReplies: () => void; openAdd: () => void }) {
  ...
  return <><section className="mt-5 flex items-center justify-between"><div><p className="text-[13px] font-medium tracking-wide text-[#64736c]">{today}</p><h2 className="mt-1 text-[25px] font-bold tracking-[-.045em]">冰箱小精灵</h2></div><button type="button" onClick={toggleVoiceReplies} className="grid h-11 w-11 place-items-center rounded-full bg-white text-[#173f35] shadow-[0_5px_16px_rgba(23,63,53,.08)] active:scale-95" aria-label={voiceReplies ? "停止并关闭语音回复" : "开启语音回复"} title={voiceReplies ? "停止并关闭语音回复" : "开启语音回复"}><Icon name={voiceReplies ? "speaker" : "mute"} /></button></section>
...
function MoreView({ initialCredentials, initialPreferences, vapidPublicKey }: { initialCredentials: CredentialSummary[]; initialPreferences: FoodPreferences; vapidPublicKey: string }) { return <section className="mt-7 grid gap-5"><div><h2 className="text-[25px] font-bold tracking-[-.04em]">更多</h2><p className="mt-1 text-sm text-[#6f8178]">偏好、提醒与模型设置</p></div><MealRecommendations initialPreferences={initialPreferences} /><NotificationControl vapidPublicKey={vapidPublicKey} /><ShelfLifeSettings /><LlmSettings initialCredentials={initialCredentials} /></section>; }
...
function VoiceButton({ disabled, onAudio, onError, onPhaseChange, phase, hero = false }: { disabled: boolean; onAudio: (audio: Blob) => void; onError: (message: string) => void; onPhaseChange: (phase: AgentPhase) => void; phase: AgentPhase; hero?: boolean }) {
  ...
  function stop() { pressed.current = false; if (recorder.current?.state === "recording") recorder.current.stop(); else { setPressing(false); onPhaseChange("idle"); } }
  const listening = pressing || recording;
  if (hero) return <button type="button" disabled={disabled} onPointerDown={(event) => void start(event)} onPointerUp={stop} onPointerCancel={stop} className={`group mt-7 flex w-full flex-col items-center rounded-[34px] px-5 py-4 outline-none transition-transform active:scale-[.98] disabled:opacity-70 ${listening ? "fridge-agent-listening" : phase === "thinking" ? "fridge-agent-thinking" : phase === "speaking" ? "fridge-agent-speaking" : ""}`} aria-label={listening ? "正在聆听，松开后发送" : disabled ? "正在整理语音" : "长按冰箱小精灵说话"}><FridgeSprite phase={phase} listening={listening} /><span className={`mt-3 rounded-full px-4 py-2 text-sm font-semibold ${listening ? "bg-rose-100 text-rose-700" : phase === "thinking" ? "bg-amber-100 text-amber-800" : phase === "speaking" ? "bg-[#dcece3] text-[#173f35]" : disabled ? "bg-[#e5e9e4] text-[#64736c]" : "bg-[#dcece3] text-[#173f35]"}`}>{listening ? "正在听，松开后发送" : phase === "transcribing" ? "正在识别…" : phase === "thinking" ? "正在整理…" : phase === "speaking" ? "正在回复，按住可打断" : disabled ? "正在整理…" : "长按说话"}</span></button>;
  return <button type="button" disabled={disabled} onPointerDown={(event) => void start(event)} onPointerUp={stop} onPointerCancel={stop} className={`grid h-10 w-10 shrink-0 place-items-center rounded-[16px] transition-transform active:scale-95 ${listening ? "bg-rose-500 text-white" : "bg-[#dcece3] text-[#173f35]"}`} aria-label={listening ? "正在聆听，松开后发送" : "按住说话"} title={listening ? "松开后发送" : "按住说话"}><Icon name="mic" /></button>;
}
