...
export async function recognizeFoodImage(file: File, householdId = "default-household"): Promise<FoodCandidate[]> {
  ...
  const systemPrompt = isQwen
    ? `识别本次采购的食物。只返回看得见或高度确定的食物；数量不确定时用 1，所有结果都是用户确认前的候选。必须只返回合法 JSON，不要 Markdown。格式：{"candidates":[{"name":"食物名","category":"蔬菜","quantity":1,"unit":"份","storageLocation":"冷藏室","opened":false}]}。category 只能是：${foodCategories.join("、")}；storageLocation 只能是：${storageLocations.join("、")}。`
    : `识别本次采购的食物。只返回看得见或高度确定的食物；数量不确定时用 1，所有结果都是用户确认前的候选。必须只返回合法 JSON，不要 Markdown。格式为 ${JSON.stringify(candidatesSchema.toJSONSchema({ target: "draft-7" }))}`;
  const response = await fetch(isQwen ? "https://dashscope.aliyuncs.com/compatible-mode/v1/chat/completions" : "https://api.openai.com/v1/chat/completions", {
    ...
    body: JSON.stringify({
      ...
      messages: [{ role: "system", content: systemPrompt }, { role: "user", content: [{ type: "text", text: "请识别这张采购食物照片。" }, { type: "image_url", image_url: { url: image } }] }],
    }),
  });
